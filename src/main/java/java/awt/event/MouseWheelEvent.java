 ...
 *
 * <p>A wheel mouse is a mouse which has a wheel in place of the middle button. This wheel can be
 * rotated towards or away from the user. Mouse wheels are most often used for scrolling, though
 * other uses are possible.
 *
 * <p>A MouseWheelEvent object is passed to every <code>MouseWheelListener</code> object which
 * registered to receive the "interesting" mouse events using the component's <code>
 * addMouseWheelListener</code> method. Each such listener object gets a <code>MouseEvent</code>
 * containing the mouse event.
 *
 * <p>Due to the mouse wheel's special relationship to scrolling Components, MouseWheelEvents are
 ...
 * ScrollPane).
 *
 * <p>MouseWheelEvents start delivery from the Component underneath the mouse cursor. If
 ...
 * destination (the ScrollPane). This allows a complex GUI to be installed without modification into
 * a ScrollPane, and for all MouseWheelEvents to be delivered to the ScrollPane for scrolling.
 *
 * <p>Some AWT Components are implemented using native widgets which display their own scrollbars
 * and handle their own scrolling. The particular Components for which this is true will vary from
 ...
 *
 * <p>Platforms offer customization of the amount of scrolling that should take place when the mouse
 ...
 * MouseWheelEvent offers methods for conforming to the underlying platform settings. These platform
 * settings can be changed at any time by the user. MouseWheelEvents reflect the most recent
 * settings.
 *
 * <p>The <code>MouseWheelEvent</code> class includes methods for getting the number of "clicks" by
 * which the mouse wheel is rotated. The {@link #getWheelRotation} method returns the integer number
 * of "clicks" corresponding to the number of notches by which the wheel was rotated. In addition to
 * this method, the <code>MouseWheelEvent</code> class provides the {@link #getPreciseWheelRotation}
...
public class MouseWheelEvent extends MouseEvent {
   ...
   * modifiers, coordinates, scroll type, scroll amount, and wheel rotation.
   *
   * <p>Absolute coordinates xAbs and yAbs are set to source's location on screen plus relative
   * coordinates x and y. xAbs and yAbs are set to zero if the source is not showing.
   *
   ...
   * constructor, the MouseWheelEvent instance is still created and no exception is thrown.
   *
   * @param source the <code>Component</code> that originated the event
   * @param id the integer that identifies the event
   * @param when a long that gives the time the event occurred
