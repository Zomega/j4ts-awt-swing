...
public class AffineTransform implements Cloneable, java.io.Serializable {
   ...
   * </pre>
   *
   * @param Tx the <code>AffineTransform</code> object to be concatenated with this <code>
   *     AffineTransform</code> object.
   * @see #preConcatenate
   * @since 1.2
   */
   ...
   * @param ptDst the specified <code>Point2D</code> that stores the result of transforming <code>
   *     ptSrc</code>
   ...
   * in the arrays starting at the specified offset in the order <code>[x0, y0, x1, y1, ..., xn, yn]
   * </code>.
   *
   ...
   * affecting the validity of the results. This method ensures that no source coordinates are
   * overwritten by a previous operation before they can be transformed. The coordinates are stored
   * in the arrays starting at the indicated offset in the order <code>[x0, y0, x1, y1, ..., xn, yn]
   * </code>.
   *
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   *     of x,&nbsp;y coordinates.
   ...
   * and then the result of the transform is stored in this object. In either case, <code>ptDst
   * </code>, which contains the transformed point, is returned for convenience. If <code>ptSrc
   * </code> and <code>ptDst</code> are the same object, the input point is correctly overwritten
