...
/**
 * An <code>Area</code> object stores and manipulates a resolution-independent description of an
 * enclosed area of 2-dimensional space. <code>Area</code> objects can be transformed and can
 * perform various Constructive Area Geometry (CAG) operations when combined with other <code>Area
 * </code> objects. The CAG operations include area {@link #add addition}, {@link #subtract
 * subtraction}, {@link #intersect intersection}, and {@link #exclusiveOr exclusive or}. See the
 * linked method documentation for examples of the various operations.
 *
 * <p>The <code>Area</code> class implements the <code>Shape</code> interface and provides full
 * support for all of its hit-testing and path iteration facilities, but an <code>Area</code> is
 * more specific than a generalized path in a number of ways:
 *
 * <ul>
 *   <li>Only closed paths and sub-paths are stored. <code>Area</code> objects constructed from
 *       unclosed paths are implicitly closed during construction as if those paths had been filled
 *       by the <code>Graphics2D.fill</code> method.
 *   <li>The interiors of the individual stored sub-paths are all non-empty and non-overlapping.
 *       Paths are decomposed during construction into separate component non-overlapping parts,
 *       empty pieces of the path are discarded, and then these non-empty and non-overlapping
 *       properties are maintained through all subsequent CAG operations. Outlines of different
 *       component sub-paths may touch each other, as long as they do not cross so that their
 *       enclosed areas overlap.
 *   <li>The geometry of the path describing the outline of the <code>Area</code> resembles the path
 *       from which it was constructed only in that it describes the same enclosed 2-dimensional
 *       area, but may use entirely different types and ordering of the path segments to do so.
 * </ul>
 *
 * Interesting issues which are not always obvious when using the <code>Area</code> include:
 *
 * <ul>
 *   <li>Creating an <code>Area</code> from an unclosed (open) <code>Shape</code> results in a
 *       closed outline in the <code>Area</code> object.
 *   <li>Creating an <code>Area</code> from a <code>Shape</code> which encloses no area (even when
 *       "closed") produces an empty <code>Area</code>. A common example of this issue is that
 *       producing an <code>Area</code> from a line will be empty since the line encloses no area.
 *       An empty <code>Area</code> will iterate no geometry in its <code>PathIterator</code>
 *       objects.
 *   <li>A self-intersecting <code>Shape</code> may be split into two (or more) sub-paths each
 *       enclosing one of the non-intersecting portions of the original path.
 *   <li>An <code>Area</code> may take more path segments to describe the same geometry even when
 *       the original outline is simple and obvious. The analysis that the <code>Area</code> class
 *       must perform on the path may not reflect the same concepts of "simple and obvious" as a
...
public class Area implements Shape, Cloneable {
   ...
   *
   * @return <code>true</code> if this <code>Area</code> object represents an empty area; <code>
   ...
   * PathIterator#SEG_MOVETO SEG_MOVETO} segments that appear in the path.
   *
   ...
   * object is unchanged.
   *
   * @param t the specified <code>AffineTransform</code> used to transform the new <code>Area</code>
   * @throws NullPointerException if <code>t</code> is null
  ...
  /**
   * Creates a {@link PathIterator} for the outline of this <code>Area</code> object. This <code>
   * Area</code> object is unchanged.
   *
...
class AreaIterator implements PathIterator {
  ...
  public int currentSegment(float coords[]) {
    ...
    int numpoints =
        (segtype == SEG_CLOSE ? 0 : (segtype == SEG_QUADTO ? 2 : (segtype == SEG_CUBICTO ? 3 : 1)));
