 ...
 * Rectangle</code> object's upper-left point {@code (x,y)} in the coordinate space, its width, and
 * its height.
 *
 * <p>A <code>Rectangle</code> object's <code>width</code> and <code>height</code> are <code>public
 * </code> fields. The constructors that create a <code>Rectangle</code>, and the methods that can
 * modify one, do not prevent setting a negative value for width or height.
 *
 * <p><a name="Empty"> A {@code Rectangle} whose width or height is exactly zero has location along
 * those axes with zero dimension, but is otherwise considered empty. The {@link #isEmpty} method
 * will return true for such a {@code Rectangle}. Methods which test if an empty {@code Rectangle}
 * contains or intersects a point or rectangle will always return false if either dimension is zero.
 * Methods which combine such a {@code Rectangle} with a point or rectangle will include the
 * location of the {@code Rectangle} on that axis in the result as if the {@link #add(Point)} method
 * were being called. </a>
 *
 * <p><a name="NonExistant"> A {@code Rectangle} whose width or height is negative has neither
 * location nor dimension along those axes with negative dimensions. Such a {@code Rectangle} is
 * treated as non-existant along those axes. Such a {@code Rectangle} is also empty with respect to
 * containment calculations and methods which test if it contains or intersects a point or rectangle
 * will always return false. Methods which combine such a {@code Rectangle} with a point or
 * rectangle will ignore the {@code Rectangle} entirely in generating the result. If two {@code
 * Rectangle} objects are combined and each has a negative dimension, the result will have at least
 * one negative dimension. </a>
 *
 * <p>Methods which affect only the location of a {@code Rectangle} will operate on its location
 * regardless of whether or not it has a negative or zero dimension along either axis.
 *
 * <p>Note that a {@code Rectangle} constructed with the default no-argument constructor will have
 ...
 * may produce a result that exceeds the range of a 32-bit integer. The methods will calculate their
...
public class Rectangle extends Rectangle2D implements Shape, java.io.Serializable {
  ...
  /**
   * Constructs a new <code>Rectangle</code> whose upper-left corner is at (0,&nbsp;0) in the
  ...
  /**
   * Constructs a new <code>Rectangle</code> whose upper-left corner is at (0,&nbsp;0) in the
   * coordinate space, and whose width and height are specified by the arguments of the same name.
   *
   * @param width the width of the <code>Rectangle</code>
   * @param height the height of the <code>Rectangle</code>
   */
  ...
  /**
   * Constructs a new <code>Rectangle</code> whose upper-left corner is specified by the {@link
   * Point} argument, and whose width and height are specified by the {@link Dimension} argument.
   *
   * @param p a <code>Point</code> that is the upper-left corner of the <code>Rectangle</code>
   * @param d a <code>Dimension</code>, representing the width and height of the <code>Rectangle
   *     </code>
   */
  ...
  /**
   * Constructs a new <code>Rectangle</code> whose upper-left corner is the specified <code>Point
   * </code>, and whose width and height are both zero.
   *
  ...
  /**
   * Constructs a new <code>Rectangle</code> whose top left corner is (0,&nbsp;0) and whose width
  ...
  /**
   * Sets the bounding <code>Rectangle</code> of this <code>Rectangle</code> to match the specified
   * <code>Rectangle</code>.
   *
  ...
  /**
   * Checks whether or not this <code>Rectangle</code> entirely contains the specified <code>
   * Rectangle</code>.
   *
   * @param r the specified <code>Rectangle</code>
  ...
  /**
   * Determines whether or not this <code>Rectangle</code> and the specified <code>Rectangle</code>
   * intersect. Two rectangles intersect if their intersection is nonempty.
   *
   * @param r the specified <code>Rectangle</code>
   * @return <code>true</code> if the specified <code>Rectangle</code> and this <code>Rectangle
   *     </code> intersect; <code>false</code> otherwise.
   */
   ...
   *
   * @param r the specified <code>Rectangle</code>
   * @return the smallest <code>Rectangle</code> containing both the specified <code>Rectangle
   ...
   *
   * <p>After adding a <code>Point</code>, a call to <code>contains</code> with the added <code>
   * Point</code> as an argument does not necessarily return <code>true</code>. The <code>contains
   * </code> method does not return <code>true</code> for points on the right or bottom edges of a
   * <code>Rectangle</code>. Therefore if the added <code>Point</code> falls on the right or bottom
   * edge of the enlarged <code>Rectangle</code>, <code>contains</code> returns <code>false</code>
   * for that <code>Point</code>. If the specified point must be contained within the new {@code
  ...
  /**
   * Adds a <code>Rectangle</code> to this <code>Rectangle</code>. The resulting <code>Rectangle
   * </code> is the union of the two rectangles.
   *
