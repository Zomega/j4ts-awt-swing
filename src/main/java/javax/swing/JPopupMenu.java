...
/**
 * An implementation of a popup menu -- a small window that pops up and displays a series of
 * choices. A <code>JPopupMenu</code> is used for the menu that appears when the user selects an
 * item on the menu bar. It is also used for "pull-right" menu that appears when the selects a menu
 * item that activates it. Finally, a <code>JPopupMenu</code> can also be used anywhere else you
 * want a menu to appear. For example, when the user right-clicks in a specified area.
 *
 * <p>For information and examples of using popup menus, see <a href=
 * "http://docs.oracle.com/javase/tutorial/uiswing/components/menu.html">How to Use Menus</a> in
...
public class JPopupMenu extends JComponent implements MenuElement {
  ...
  public MenuElement[] getSubElements() {
    ...
    for (i = 0; i < c; i++) {
      m = getComponent(i);
      if (m instanceof MenuElement) tmp.addElement((MenuElement) any(m));
    }
