 ...
 * displayed when the user selects an item on the <code>JMenuBar</code>. In addition to <code>
 * JMenuItem</code>s, a <code>JMenu</code> can also contain <code>JSeparator</code>s.
 *
 * <p>In essence, a menu is a button with an associated <code>JPopupMenu</code>. When the "button"
 * is pressed, the <code>JPopupMenu</code> appears. If the "button" is on the <code>JMenuBar</code>,
 * the menu is a top-level window. If the "button" is another menu item, then the <code>JPopupMenu
 * </code> is "pull-right" menu.
 *
 * <p>Menus can be configured, and to some degree controlled, by <code>
 * <a href="Action.html">Action</a></code>s. Using an <code>Action</code> with a menu has many
 * benefits beyond directly configuring a menu. Refer to <a href="Action.html#buttonActions">Swing
 * Components Supporting <code>Action</code></a> for more details, and you can find more information
 * in <a href="http://docs.oracle.com/javase/tutorial/uiswing/misc/action.html">How to Use
 * Actions</a>, a section in <em>The Java Tutorial</em>.
 *
 * <p>For information and examples of using menus see <a href=
 * "http://docs.oracle.com/javase/tutorial/uiswing/components/menu.html">How to Use Menus</a>, a
 * section in <em>The Java Tutorial.</em>
 *
...
public class JMenu extends JMenuItem implements MenuElement {
  ...
  /**
   * Creates a new menu item attached to the specified <code>Action</code> object and appends it to
  ...
  /**
   * Returns the <code>java.awt.Component</code> used to paint this <code>MenuElement</code>. The
   * returned component is used to convert events and detect if an event is inside a menu component.
   */
