...
/**
 * StateEdit is a general edit for objects that change state. Objects being edited must conform to
 * the StateEditable interface.
 *
 * <p>This edit class works by asking an object to store it's state in Hashtables before and after
 * editing occurs. Upon undo or redo the object is told to restore it's state from these Hashtables.
 ...
 * </PRE>
 *
 * <p><EM>Note that when a StateEdit ends, it removes redundant state from the Hashtables - A state
 * Hashtable is not guaranteed to contain all keys/values placed into it when the state is
 * stored!</EM>
 *
 * @see StateEditable
 * @author Ray Ryan
