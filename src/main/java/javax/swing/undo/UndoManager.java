...
/**
 * {@code UndoManager} manages a list of {@code UndoableEdits}, providing a way to undo or redo the
 * appropriate edits. There are two ways to add edits to an <code>UndoManager</code>. Add the edit
 * directly using the <code>addEdit</code> method, or add the <code>UndoManager</code> to a bean
 * that supports <code>UndoableEditListener</code>. The following examples creates an <code>
 * UndoManager</code> and adds it as an <code>UndoableEditListener</code> to a <code>JTextField
 * </code>:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><code>UndoManager</code> maintains an ordered list of edits and the index of the next edit in
 * that list. The index of the next edit is either the size of the current list of edits, or if
 * <code>undo</code> has been invoked it corresponds to the index of the last significant edit that
 * was undone. When <code>undo</code> is invoked all edits from the index of the next edit to the
 * last significant edit are undone, in reverse order. For example, consider an <code>UndoManager
 * </code> consisting of the following edits: <b>A</b> <i>b</i> <i>c</i> <b>D</b>. Edits with a
 * upper-case letter in bold are significant, those in lower-case and italicized are insignificant.
 *
 * <p><a name="figure1"></a>
 *
 ...
 * </table>
 *
 * <p>As shown in <a href="#figure1">figure 1</a>, if <b>D</b> was just added, the index of the next
 * edit will be 4. Invoking <code>undo</code> results in invoking <code>undo</code> on <b>D</b> and
 * setting the index of the next edit to 3 (edit <i>c</i>), as shown in the following figure.
 *
 * <p><a name="figure2"></a>
 *
 * <table border=0 summary="">
 ...
 * </table>
 *
 * <p>The last significant edit is <b>A</b>, so that invoking <code>undo</code> again invokes <code>
 * undo</code> on <i>c</i>, <i>b</i>, and <b>A</b>, in that order, setting the index of the next
 * edit to 0, as shown in the following figure.
 *
 * <p><a name="figure3"></a>
 *
 * <table border=0 summary="">
 ...
 * </table>
 *
 * <p>Invoking <code>redo</code> results in invoking <code>redo</code> on all edits between the
 * index of the next edit and the next significant edit (or the end of the list). Continuing with
 * the previous example if <code>redo</code> were invoked, <code>redo</code> would in turn be
 * invoked on <b>A</b>, <i>b</i> and <i>c</i>. In addition the index of the next edit is set to 3
 * (as shown in <a href="#figure2">figure 2</a>).
 *
 * <p>Adding an edit to an <code>UndoManager</code> results in removing all edits from the index of
 * the next edit to the end of the list. Continuing with the previous example, if a new edit,
 * <i>e</i>, is added the edit <b>D</b> is removed from the list (after having <code>die</code>
 * invoked on it). If <i>c</i> is not incorporated by the next edit (<code>
 * <i>c</i>.addEdit(<i>e</i>)</code> returns true), or replaced by it (<code>
 * <i>e</i>.replaceEdit(<i>c</i>)</code> returns true), the new edit is added after <i>c</i>, as
 ...
 * </table>
 *
 * <p>Once <code>end</code> has been invoked on an <code>UndoManager</code> the superclass behavior
 * is used for all <code>UndoableEdit</code> methods. Refer to <code>CompoundEdit</code> for more
 * details on its behavior.
 *
 * <p>Unlike the rest of Swing, this class is thread safe.
 *
