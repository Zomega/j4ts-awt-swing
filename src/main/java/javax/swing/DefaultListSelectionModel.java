...
public class DefaultListSelectionModel implements ListSelectionModel, Cloneable, Serializable {
  ...
  /**
   * Notifies <code>ListSelectionListeners</code> that the value of the selection, in the closed
   * interval <code>firstIndex</code>, <code>lastIndex</code>, has changed.
   */
  ...
  private void changeSelection(
      int clearMin, int clearMax, int setMin, int setMax, boolean clearFirst) {
  ...
  /**
   * Change the selection with the effect of first clearing the values in the inclusive range
   * [clearMin, clearMax] then setting the values in the inclusive range [setMin, setMax]. Do this
   * in one pass so that no values are cleared if they would later be set.
   */
  ...
  public void insertIndexInterval(int index, int length, boolean before) {
    ...
    boolean setInsertedValues =
        ((getSelectionMode() == SINGLE_SELECTION) ? false : value.get(index));
