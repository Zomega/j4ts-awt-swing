 ...
 * <code>JMenuItem</code> contained in a <code>JPopupMenu</code> performs exactly that function.
 *
 * <p>Menu items can be configured, and to some degree controlled, by <code>
 * <a href="Action.html">Action</a></code>s. Using an <code>Action</code> with a menu item has many
 * benefits beyond directly configuring a menu item. Refer to <a href="Action.html#buttonActions">
 * Swing Components Supporting <code>Action</code></a> for more details, and you can find more
 * information in <a href="http://docs.oracle.com/javase/tutorial/uiswing/misc/action.html">How to
 * Use Actions</a>, a section in <em>The Java Tutorial</em>.
 *
 * <p>For further documentation and for examples, see <a
...
public class JMenuItem extends AbstractButton implements MenuElement {
  ...
  /**
   * Creates a menu item whose properties are taken from the specified <code>Action</code>.
   *
  ...
  /**
   * Creates a <code>JMenuItem</code> with the specified text and keyboard mnemonic.
   *
   * @param text the text of the <code>JMenuItem</code>
  ...
  /**
   * Sets the key combination which invokes the menu item's action listeners without navigating the
   * menu hierarchy. It is the UI's responsibility to install the correct action. Note that when the
  ...
  /**
   * Returns the <code>java.awt.Component</code> used to paint this object. The returned component
   * will be used to convert events and detect if an event is inside a menu component.
   *
